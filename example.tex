...
The dynamic adaptation of \texttt{C1} and \texttt{C2} % These are not
                                % represented in the Figure - JJ
...
techniques \cite{Baldini2016287} %to what? - JJ
...
These components % which components? The previous paragraph talked about
                 % applications, if you mean components of the application say
                 % so - JJ
are easily implemented and deployed to cloud environments, where
processing nodes can be defined and deployed using scripts, % best practices
                                % would say that you should use continuous
                                % deployment procedures (including scripts,
                                % Ansible or things like that - JJ
...
specifically designed for deployment in Amazon's AWS. % Please highlight
                                % differences - JJ
...
have not fully leveraged the advancements in cloud-native technologies. %You
                                %will have to prove you obtain better results in
                                %some aspect that in those papers - JJ
...
computational loads. %Maybe add something about replicability, and the fact that
                     %it's open source - JJ
...
\begin{table}[ht]
\small
\caption{Cost AWS} \label{tab:costos} %Probably add something about the role of
                                %every service - JJ
...
Fargate Ephemeral Storage GB per hour   &  \$0.000111   &  20 GB \\ % But are
                                % you using this or Fargate Tasks? Is it the
                                % same cost?
\hline
...
\begin{table*}[ht]
\centering
\caption{Experimental Setup}\label{tab:alg_params} % Please say something about
                                % how these values are obtained - JJ
...
Cloud       & Size              & 10       \\
  ...
  % Also why these values are different. Make captions as self-explanatory as
  % possible - JJ
Local       & Size              & 10       \\
            & Swarms            & 16       \\
            & Iterations        &  8       \\
...
%This is a bit confusing, unless you're looking at the figure all the time, and
%the Figure is quite far. Also, this explanation should be as self contained as
%possible, even with references. Maybe start this paragraph with "These
%multi-swarms are combined using the Combinator, shown in the left hand side of
%Figure 1 (general concept) and dashed block of Figure 2 (actual
%implementation)" - JJ
...
with in a range of values. This is called a heterogeneous strategy, % reference
                                % - JJ
this strategy 
...
\texttt{input-queue}. % Other serialization formats might be possible, and
                      % probably have higher throughput; formats like feather or
                      % other used in machine learning. It's probably OK if no
                      % heavy reviewing is going to be applied, but bear in mind
                      % for the future - JJ
...
cycle. % First mention of C1 and C2, maybe a bit of explanation is needed - JJ
...
directly. % this is the first reference, maybe explain why you need it - JJ
...
the progression in seconds, indicating the duration of each PSO instance's activity 
inside a particular worker container.} % Maybe explain why some workers
                                % terminate early, and the kind of conclusion
                                % one should gather from this - JJ
\label{fig:36w40s}
\end{figure*}

Each of the services mentioned above has a cost that is charged monthly, the current 
prices are shown in Table \ref{tab:costos}. % Maybe show the actual cost?
                                % Compare it with the amortization cost of the
                                % MacStudio + electricity consumption (Maybe a
                                % reference to our ICSoft paper? - JJ
...
a local execution using a Mac Studio workstation. % Maybe add more details:
                                % processor, threads, so on - JJ
